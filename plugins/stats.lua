...
local function run(msg, matches)
  if matches[1]:lower() == 'octa' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /octa ")
    return about
  end 
  if matches[1]:lower() == "statslist" then
    if not is_momod(msg) then
      return "⛔️ For mods only !"
    end
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "octa" then -- Put everything you like :)
      if not is_admin1(msg) then
        return "⛔️ For admins only !"
      else
        return bot_stats()
      end
    end
