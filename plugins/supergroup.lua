...
local title ="<b>Info for:</b><code> ["..result.title.."]</code>\n\n"
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return "⛔️ <b>For moderators only!</b>"
  end
...
local function modlist(msg)
  ...
  local i = 1
  local message = '\n<b>List of moderators for </b><code>' .. string.gsub(msg.to.print_name, '_', ' ') .. '</code><b> :</b>\n'
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '⛔️ <b>Error:</b><code> Failed to retrieve link </code>\n🔴 <b>Reason:</b><code> Not creator.</code>\n\n🔹 <b>If you have the link, please use #setlink to set it</b>')
		...
		if matches[1] == 'demote' then
			...
			if not is_owner(msg) then
				return "⛔️ <b>Only owner/support/admin can promote</b>"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return '🔵 <b>Send the new group photo now</b>'
		end
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "✅ <b>Mute</b> <code>"..msg_type.."</code><b>  has been enabled</b>"
				else
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "<b>Message #superhelp to </b>@OctaB<b> in private for SuperGroup help. \nOr visit: </b>www.antiispam.com/help/supergroup ."
